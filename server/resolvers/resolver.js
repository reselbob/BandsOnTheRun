...
module.exports = {
    ...
    Mutation: {
        ...
        updateMusician: async (root, args) => {
            const data  = await Musican.findOneAndUpdate({_id: args.id}, {$set:{instruments:args.instruments}}, {new: true});
            await eventPublisher.publish('EVENT_ADDED', 'MUSICIAN_UPDATED', data);
            return data;lt
        },
        updateBandMember: async (root, args) => {
            const data  = await BandMember.findOneAndUpdate({_id: args.id},
                {$set:{bandId: args.bandId, musicianId : args.musicianId, startDate : args.startDate, endDate: args.endDate} }, {new: true});
            await eventPublisher.publish('EVENT_ADDED', 'BANDMEMBER_UPDATED', data);
            return data;
        },
        updateBand: async (root, args) => {
            const data  = await Band.findOneAndUpdate({_id: args.id}, {$set:{name:args.name, genre:args.genre }}, {new: true});
            await eventPublisher.publish('EVENT_ADDED', 'BAND_UPDATED', data);
            return data;

        },
        updateSong: async (root, args) => {
            const data  = await Song.findOneAndUpdate({_id: args.id}, {$set:{name:args.title, runtime:args.runtime, musicianIds: args.musicianIds} }, {new: true});
            await eventPublisher.publish('EVENT_ADDED', 'SONG_UPDATED', data);
            return data;
        },
        updateAlbum: async (root, args) => {
            const data  = await Album.findOneAndUpdate({_id: args.id}, {$set:{name:args.title, releaseDate:args.releaseDate, songIds: args.songIds} }, {new: true});
